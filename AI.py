...
class AI:
    ...
    def pick(self, world):
        ...
        my_deck = [all_base_units[4],all_base_units[1],all_base_units[6],all_base_units[0],all_base_units[8]]
        ...
        print("hello")
        print(world.get_me().deck)
        print("hello")

        # other pre process
        self.path_for_my_units = world.get_friend().paths_from_player[0]
        print(world.get_me().paths_from_player[0])
        ...
        if received_spell is not None:
            if received_spell.is_area_spell():
                if received_spell.target == SpellTarget.ENEMY:
                    ...
                    if len(enemy_units) > 0: #miad be avalin unite in spell ro asar mide (ma mitoonim be yeki dg asar bedim!)
